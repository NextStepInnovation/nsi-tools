'''Functional graph analysis tools
...
GraphFunction = T.Callable[[T.Any], nx.Graph]
...
def neighbor_selector(selectors: T.Iterable[Selector],
    ...
    return _.pipe(
        graph[node_id].items(),
        _.vfilter(lambda node_id, edge_data: all(
...
def neighbors_of_type(type, graph, node_id, *,
    ...
    return _.pipe(
        graph[node_id].items(),
        _.vfilter(lambda nid, edata: node_type(graph, nid) == type),
        _.vfilter(edge_selector),
        _.map(_.first),
    )
...
def to_function_space(f_space: T.Union[type, FunctionSpace,
    ...
    return _.pipe(
        f_space.__dict_.items(),
        _.vfilter(lambda k, v: not (k.startswith('__') and k.endswith('__'))),
        dict,
    )
...
class Graph(nx.DiGraph):
    ...
    def __call__(self, *node_ids):
        if len(node_ids) == 1 and _.is_seq(node_ids[0]):
