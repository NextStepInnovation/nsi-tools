...
from .toolz import (
    ...
    maybe_pipe, vmap, maybe_first, noop, is_dict, vcall,
)
from . import logging
    ...
    def maybe_method(self, name, **orig_kw):
        ...
        def caller(*a, **kw):
            try:
                ...
                return Outcome(False, Nothing(), error)
        return caller
...
class IdResourceEndpoint(Endpoint):
    def __init__(self, parent: Endpoint, data: dict, form_key: str,
                 id_key: str,
                 meta_f: T.Callable[[dict], dict] = empty_dict,
                 unpack_f: T.Callable[[dict], dict] = noop,
                 single_unpack_f: T.Callable[[dict], dict] = noop):
        self.parent = parent
...
def get_id_resource(resource_name: str, *,
                    ...
                    unpack_f=noop, single_unpack_f=noop,
                    help=None, **iter_kw):
    @curry
    def getter(parent_endpoint: IdResourceEndpoint, id: T.Union[int, str], 
               **get_kw):
        return pipe(
...
def new_id_resource(resource_name: str, *,
                    ...
                    meta_f=empty_dict, unpack_f=noop,
                    single_unpack_f=noop,
                    post_unpack_f=noop,
                    body_transform=noop):
